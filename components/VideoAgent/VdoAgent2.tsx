...
export default function VideoAgent({ onClose, onLoaded }: { onClose?: () => void, onLoaded?: () => void }) {
    ...
    const [toggleProductPopup, setToggleProductPopup] = useState(false);
    ...
    const audioWebSocket = useRef<WebSocket | null>(null);
    ...
    const startAudioStream = () => {
        ...
        // Wait until the user has joined the Daily room before starting audio socket
        dailyCall.current.on('joined-meeting', () => {
            ...
            socket.addEventListener('error', (event) => {
                ...
                updateStatus(`Audio WebSocket error: ${event || "Unknown error"}`);
            });

            socket.addEventListener('close', (event) => {
                console.log('Audio WebSocket connection closed:', event);
                updateStatus(`Audio WebSocket closed. Code: ${event.code}, Reason: ${event.reason}, Clean: ${event.wasClean}`);
            });
                    ...
                    if (data.content) {
                        ...
                        const productNames =
                            data.products && Array.isArray(data.products)
                                ? data.products
                                : extractProductNamesFromTranscript(data.content);

                        console.log('[Product Names]', productNames);
                        if (productNames && productNames?.length > 0) {
                            const matchedProd = matchProducts(productNames, products);
                            console.log("matched", matchedProd);
                            setProductRecommendations((prev) => [...prev, ...matchedProd]);
                            setMatchedProducts(matchedProd);
                            setTitle(title || "Product Recommendations");
                            console.log("title", title);
                            if (isPhone) {
                                router.push("/recommended");
                            } else {
                                setToggleProductPopup(true);
                            }
                        }
    ...
    return (
        <>
            <div className=" w-full h-full rounded-md md:rounded-xl shadow-lg flex flex-col items-center z-50 relative bg-white">
                {loading && (
                    <div className="bg-white rounded-xl z-20 flex items-center justify-center absolute inset-0 h-full w-full">
                        <ProductLoader className="text-sm" title={funMessages[loadingMsgIndex]} description="" />
                        {/* <p className='text-xs text-center    md:text-sm'>Summoning your virtual assistant...</p> */}
                    </div>
                )}
            ...
            <ProductPopup onClose={() => setToggleProductPopup(false)} title={title} isOpen={toggleProductPopup} products={productRecommendations} />
        </>
    );
