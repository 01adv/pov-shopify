...
export default function VideoAgent({ onClose }: { onClose?: () => void }) {
    ...
    const API_CONFIG = {
        apiKey: "NGZiNTliNTEwODRkNGM0Zjg1ODUyMDkyYTgyZGFhOTItMTczMzAxODI1Nw==",
    ...
    return (
        <div className="fixed top-4 bottom-2 right-4 w-32 h-52 bg-white rounded-md shadow-lg flex flex-col items-center z-50">
            {loading && (
            ...
            <video
                ref={videoRef}
                className="w-32 h-52 rounded-md shadow-lg object-cover"
                autoPlay
                playsInline
            ...
            <button
                onClick={toggleMic}
                className="absolute bottom-2 right-2 bg-blue-600 text-white text-xs px-2 py-1 rounded-full shadow-md hover:bg-blue-700 transition"
            >
                {isMicOn ? "⏸" : "▶"}
            </button>
        </div>
